    ...
    for num, task in enumerate(commands[1:]):
        ...
        if task['location'] == 'remote':
            ...
            if task['hostname'] not in remote_prep_done:
                ...
                remote_clients[task['hostname']] = \
                    RemoteClient.RemoteClient(task['hostname'], task['username'], task['basefolder'])
                remote_folders[task['hostname']] = \
                    create_remote_workflow_path(workflow_id, remote_clients[task['hostname']])
...
def create_remote_workflow_path(workflow_id, RemoteClient):
    ...
    RemoteClient.execute_commands(Path(RemoteClient.remote_path), \
        [f'mkdir {remote_workflow_path}'])
